...
func (object Object) Valid() bool {
	...
	case SevGuest:
		if object.ID == "" || object.CBitPos == 0 || object.ReducedPhysBits == 0 {
			return false
		}
...
func (config *Config) appendBios() {
	if config.Bios != "" {
		if config.Knobs.MemEncrypt {
			config.qemuParams = append(config.qemuParams, "-drive")
			config.qemuParams = append(config.qemuParams, fmt.Sprintf("if=pflash,format=raw,unit=0,file=%s,readonly=on", config.Bios))
		} else {
